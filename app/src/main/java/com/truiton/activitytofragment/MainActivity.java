...
public class MainActivity extends AppCompatActivity implements IFragmentToActivity {
    ...
    @Override
    public void communicateToTravel(String pointer, String data) {
        travelTab Travelfragment = (travelTab) adapter.getFragment(0);
        if (Travelfragment != null) {
            Travelfragment.fragmentCommunication(pointer,data);
        } else {
    ...
    @Override
    public void communicateToLocation(String pointer, String data) {
        locationTab Locationfragment = (locationTab) adapter.getFragment(1);
        if (Locationfragment != null) {
            Locationfragment.fragmentCommunication(pointer,data);
        } else {
    ...
    @Override
    public void communicateToInventory(String pointer, String data) {
        inventoryTab Inventoryfragment = (inventoryTab) adapter.getFragment(2);
        if (Inventoryfragment != null) {
            Inventoryfragment.fragmentCommunication(pointer,data);
        } else {
